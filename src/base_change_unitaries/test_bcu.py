...
class TestGellMann(unittest.TestCase):
    ...
    def test_inverse_relation(self):
        from src.base_change_unitaries.bcu_el_gm import bcu_el_gm
        from src.base_change_unitaries.bcu_gm_el import bcu_gm_el
    ...
    def test_traceless_hermitian(self):
        ...
        for dim in range(2, max_dim):
            ugm = bcu_gm_el(dim).reshape((dim, dim, dim**2))
...
if __name__ == "__main__":
    ...
    from bcu_gm_el import bcu_gm_el
    dim = 3
    ugm = bcu_gm_el(dim).reshape((dim, dim, dim**2))

    print(np.trace(ugm, axis1=0, axis2=1))
    print()

...
class TestChannelOperations(unittest.TestCase):
    ...
    def test_choi_state(self):
        ...
        choi_trdiff = co_choi_state(trdiff)
        ...
        choi_tdeph = co_choi_state(tdeph)
        ...
        choi_tdepol = co_choi_state(tdepol)
    ...
    def test_fixed_points(self):
        from src.channel_operations.co_fixed_points import co_fixed_points
        ...
        with self.assertRaises(ValueError):
            co_fixed_points(tamply)
        
    def test_tensor(self):
        from src.channel_operations.co_tensor import co_tensor
        dim = 3
        dim2 = dim + 1

        # Product of Unitary channels -> again unitary
        tu = self._unitary_channel(dim)
        tensor_tu = co_tensor(tu, 2)
        ...
        tensor_tdephas = co_tensor(tdephas, 2)
        ...
        p = .5
        tdepol = self._depolarizing_channel(dim, p)
        tenspor_depol = co_tensor(tdepol, 2)
        ...
        tensor_3 = co_tensor([tdepol, tdephas, tu])
